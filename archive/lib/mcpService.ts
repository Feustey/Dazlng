...
export class McpService {
  ...
  async predictNodeGrowth(nodePubkey: string): Promise<NodeGrowthPrediction> {
    try {
      const response = await fetch(
        `${this.baseUrl}/api/predictions/node-growth/${nodePubkey}`
      );
      if (!response.ok) {
      ...
      console.error(
        "Erreur lors de la prédiction de la croissance du nœud:",
        error
      );
      throw error;
    }
  ...
  async predictNetworkTrends(): Promise<NetworkTrendsPrediction> {
    try {
      const response = await fetch(
        `${this.baseUrl}/api/predictions/network-trends`
      );
      if (!response.ok) {
      ...
      console.error(
        "Erreur lors de la prédiction des tendances du réseau:",
        error
      );
      throw error;
    }
  ...
  async analyzeFeeMarket(): Promise<FeeMarketAnalysis> {
    try {
      const response = await fetch(`${this.baseUrl}/api/analysis/fee-market`);
      if (!response.ok) {
