...
async function testProductionOTP(): Promise<void> {
  ...
  try {
    ...
    if (sendData.success) {
      ...
      console.log(`curl -X POST ${baseUrl}/api/otp/verify-code -H "Content-Type: application/json" -d '{"email":"stephane.courant@pm.me","code":"VOTRE_CODE","name":"Stéphane Courant"}'`);
