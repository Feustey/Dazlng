import { QueryOptimizer } from "../../app/lib/services/queryOptimizer";
...
describe("QueryOptimizer", () => {
  ...
  describe("getNodeWithOptimizedQuery", () => {
    it("devrait retourner un nœud depuis la base de données", async () => {
      const result =
        await queryOptimizer.getNodeWithOptimizedQuery("test-pubkey");

      expect(connectToDatabase).toHaveBeenCalled();
      expect(Node.findOne).toHaveBeenCalledWith({ pubkey: "test-pubkey" });
      expect(result).toEqual(mockNode);
    });

    it("devrait utiliser le cache si disponible et valide", async () => {
      const cacheKey = queryOptimizer["getCacheKey"]("getNode", {
        pubkey: "test-pubkey",
      });
    ...
    it("devrait gérer les erreurs de requête", async () => {
      ...
      await expect(
        queryOptimizer.getNodeWithOptimizedQuery("test-pubkey")
  ...
  describe("getTopNodes", () => {
    ...
    it("devrait utiliser le cache si disponible et valide", async () => {
      const cacheKey = queryOptimizer["getCacheKey"]("getTopNodes", {
        limit: 10,
      });
      queryOptimizer["queryCache"].set(cacheKey, {
        data: mockNodes,
  ...
  describe("getPeersOfPeersWithOptimizedQuery", () => {
    ...
    it("devrait retourner les pairs depuis la base de données", async () => {
      const result =
        await queryOptimizer.getPeersOfPeersWithOptimizedQuery("test-pubkey");
    ...
    it("devrait utiliser le cache si disponible et valide", async () => {
      const cacheKey = queryOptimizer["getCacheKey"]("getPeersOfPeers", {
        nodePubkey: "test-pubkey",
      });
      queryOptimizer["queryCache"].set(cacheKey, {
        data: mockPeers,
    ...
    it("devrait gérer les erreurs de requête", async () => {
      ...
      await expect(
        queryOptimizer.getPeersOfPeersWithOptimizedQuery("test-pubkey")
      ).rejects.toThrow("DB Error");
    });
  });
  ...
  describe("clearCache", () => {
    it("devrait vider le cache", () => {
      queryOptimizer["queryCache"].set("test-key", {
        data: {},
        timestamp: Date.now(),
      });
