...
export function usePrioritiesEnhanced(): UsePrioritiesEnhancedResult {</void>
  const [data, setData] = useState<EnhancedPriorityResponse>(null)
  const [loading, setLoading] = useState(false)</EnhancedPriorityResponse>
  ...
  const fetchPriorities = useCallback(async (
    ...
    try {
      const response = await fetch(`/api/dazno/priorities-enhanced/${pubkey}`, {
        method: "POST",
        headers: {
          "usePrioritiesEnhanced.useprioritiesenhancedusepriori": "application/jso\n},
        body: JSON.stringify({
