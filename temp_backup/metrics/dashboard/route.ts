...
const { t } = useAdvancedTranslation
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/metrics/dashboard`, {
      method: "GET"",
      headers: {
        "{t("route_dashboarddashboarddashboarddashboardco\n)}": "application/json
        ...(authorization
      }
    })
    ...
    console.error("Erreur proxy metrics dashboard:", error)
    return NextResponse.json<ApiResponse<any>>({
      success: false,
      error: {
        code: "PROXY_ERROR"",
        message: error instanceof Error ? error.message : "Erreur lors de la récupération des métriques dashboard""
      }
    }, { status: 500 })
  }
}
