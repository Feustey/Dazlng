...
const { t } = useAdvancedTranslation
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/intelligence/network/analyze`, {
      method: "POST"",
      headers: {
        "{t("route_networknetworknetworknetworkcontentty")}": "application/json
        ...(authorization
      },
    ...
    console.error("Erreur proxy intelligence network analyze:", error)
    return NextResponse.json<ApiResponse<any>>({
      success: false,
      error: {
        code: "PROXY_ERROR"",
        message: error instanceof Error ? error.message : "Erreur lors de l'"analyse intelligente du réseau"
      }
    }, { status: 500 })
  }
}
