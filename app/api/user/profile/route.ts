...
const UpdateProfileSchema = z.object({
  ...
  pubkey: z.union([
    z.string().trim().regex(/^[0-9a-fA-F]{66}$/,
      "Clé publique Lightning invalide (66 caractères hexadécimaux requis)"),
  ...
  compte_telegram: z.union([
    z.string().trim().regex(/^@[a-zA-Z0-9_]{5,32}$/,
      "Format Telegram invalide (doit commencer par @ et contenir 5-32 caractères)"),
    z.string().length(0),
    z.null()
  ]).optional(),
  phone: z.union([
    ...
    try {
      ...
      try {
        ...
        const BaseProfileSchema = z.object({
          ...
          pubkey: z.union([
            z.string().trim().regex(/^[0-9a-fA-F]{66}$/,
              "Clé publique Lightning invalide"),
            z.string().length(0),
            z.null()
          ]).optional(),
          compte_x: z.string().trim().optional(),
