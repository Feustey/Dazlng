...
export default function PredictionModule() {
  ...
  // Charger les prédictions de croissance d'un nœud
  const handleFetchNodeGrowth = async () => {
    if (!nodePubkey.trim()) {
      setError("Veuillez entrer une clé publique valide");
      return;
    }
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        // En développement, utiliser des données mockées avec délai simulé
        setTimeout(() => {
          setNodeGrowth({
            ...mockNodeGrowthPrediction,
            pubkey: nodePubkey,
            timeframe,
          });
        ...
        const prediction = await mcpService.predictNodeGrowth(
          nodePubkey,
          timeframe
        );
      ...
      setError(`Erreur lors de la récupération des prédictions: ${err}`);
      setLoadingNode(false);
    }
  };

  // Charger les tendances du réseau
  const handleFetchNetworkTrends = async () => {
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        // En production, utiliser l'API réelle
        const mcpService = McpService.getInstance();
        const prediction = await mcpService.predictNetworkTrends(timeframe);
        setNetworkTrends(prediction);
        setLoadingNetwork(false);
      }
    } catch (err) {
      setError(`Erreur lors de la récupération des tendances: ${err}`);
      setLoadingNetwork(false);
    }
  };

  // Charger l'analyse du marché des frais
  const handleFetchFeeMarket = async () => {
    try {
      ...
      setError(`Erreur lors de la récupération de l'analyse des frais: ${err}`);
      setLoadingFees(false);
    }
  };
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex justify-end">
                  <Button onClick={handleFetchFeeMarket} disabled={loadingFees}>
                    {loadingFees ? (
                      <>
                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                        Chargement
