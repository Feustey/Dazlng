...
export const DaznoInsights: React.FC<DaznoInsightsProps> = ({
  ...
  const groupedRecommendations = recommendations.reduce((acc: Record<string, ExtendedRecommendation[]>, rec: ExtendedRecommendation) => {
      ...
      {categoryScores.map(({ category, avgConfidence }) => (
        <div key={category} className="bg-white rounded-xl shadow p-6">
          ...
          <div className="space-y-4">
            {groupedRecommendations[category].map((rec: ExtendedRecommendation) => (
