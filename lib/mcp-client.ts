...
export class MCPClient {
  ...
  async getNetworkSummary(): Promise<import('./network-types').NetworkSummary> {
    ...
    return this.fetchRAG("Donne-moi un résumé général du réseau Lightning avec les métriques principales") as Promise<import('./network-types').NetworkSummary>;
  }
  ...
  async optimizeNode(nodeId: string): Promise<import('./network-types').OptimizationResult> {
    ...
    return this.getNodeRecommendations(nodeId) as Promise<import('./network-types').OptimizationResult>;
  }
}
