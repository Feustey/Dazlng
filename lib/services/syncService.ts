...
export class SyncService {
  ...
  private async syncWithRetry<T>(operation: () => Promise<T>, retries = this.MAX_RETRIES): Promise<T> {
    try {
  ...
  async syncPeersOfPeers(): Promise<void> {
    try {
      ...
      for (const node of nodes) {
        const { peers_of_peers } = await mcpService.getPeersOfPeers(node.pubkey);
  ...
  startPeriodicSync(): void {
    if (this.syncInterval) {
      console.log('Synchronisation périodique déjà active');
      return;
    }
