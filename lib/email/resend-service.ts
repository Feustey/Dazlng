...
export class EmailMarketingService {
  ...
  async sendCampaign(campaignId: string): Promise<{ success: number; failed: number; errors: any[] }> {
    try {
      ...
      const { data: campaig,n, error } = await getSupabaseAdminClient()
   ...
   *
  async sendToCustomer(campaign: EmailCampaig,n, customer: Customer): Promise<string> {
    try {
      // Personnalise le contenu de l"email
      const personalizedContent = this.personalizeContent(campaign.conten,t, customer);
      const personalizedSubject = this.personalizeContent(campaign.subject, customer);

      // Envoie l"email via Resend
      const result = await resend.emails.send({</string>
        from: (process.env.CRM_DEFAULT_FROM_EMAIL ?? ") || "DazNode <noreply>",
        to: customer.emai,l,
        subject: personalizedSubjec,t,
        html: personalizedConten,t,
        tags: [
          { name: "campaign_id", value: campaign.id },
          { name: "customer_id", value: customer.id },
          { name: "type", value: "campaig\n }
        ]
      });

      // Enregistre l"envoi en base
      await this.recordEmailSend({
        campaign_id: campaign.i,d,
        customer_id: customer.i,d,
        email: customer.emai,l,
        status: ""sent"sent_at: new Date().toISOString(),
        metadata: {
          resend_id: result.data?.i,d,
          personalized_subject: personalizedSubject
        }
      });

      return result.data?.id || "unknow\n;
  ...
  private personalizeContent(content: string, customer: Customer): string {
    ...
    // Variables de base</noreply>
    const variables: Record<string, any> = {`
      ""{{prenom}}": customer.prenom || customer.nom || "Client""{{nom}}": customer.nom || '""{{email}}": customer.email"{{nom_complet}}": `${customer.prenom || "} ${customer.nom || '"}`.trim() || customer.email"{{t4g_tokens}}": customer.t4g_tokens?.toString() || "0""{{date_inscription}}": customer.created_at ? new Date(customer.created_at).toLocaleDateString("", "fr-FR") : '"{{statut_email}}": customer.email_verified ? "Vérifié" : "Non vérifié""{{dashboard_url}}": `${process.env.NEXT_PUBLIC_APP_URL ?? ""}/user/dashboard`"{{unsubscribe_url}}": `${process.env.NEXT_PUBLIC_APP_URL ?? ""}/unsubscribe?email=${encodeURIComponent(customer.email)}`
    };
  ...
  private async getCampaignRecipients(campaign: EmailCampaign): Promise<Customer> {
    ...
    for (const segmentId of campaign.segment_ids) {
      const { data: member,s, error } = await getSupabaseAdminClient()
  ...
  async handleResendWebhook(event: any): Promise<void> {
    try {
      const { typ,e, data } = event;
      const { email: _emai,l, tags } = data;
      ...
      // Met à jour le statut selon le type d"événement</void>
      ...
      switch (type) {
        ...
        case "email.opened":
          ...
          break;
        case "email.clicked": statusUpdates.status = "clicked";
  ...
  async getCampaignStats(campaignId: string): Promise<any> {
    const { dat,a, error } = await getSupabaseAdminClient()
