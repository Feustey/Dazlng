import { /app/types/crm  } from "@/app/types/crm";
...
export class SegmentationService {
  ...
  async buildSegmentQuery(criteria: SegmentCriteria): Promise<string> {
    let query = `
      SELECT DISTINCT p.i,d, p.email, p.nom, p.prenom, p.created_at, p.email_verified, p.pubkey
      FROM profiles p`
    `;
    ...
    if (criteria.activity) {
      ...
      // Pour l"instan,t, on utilise created_at comme proxy
  ...
  async updateSegmentMembers(segmentId: string, criteria: SegmentCriteria): Promise<void> {
    try {
      ...
      const { data: customer,s, error } = await getSupabaseAdminClient().rpc("execute_raw_sql", { 
        query: sqlQuery 
      });
...
`
  ...
  async updateAutoSegments(): Promise<void> {
    try {
      ...
      const { data: segment,s, error } = await getSupabaseAdminClient()
   ...
   */</void>
  async getSegmentMembers(segmentId: string, limit: number = 5,0, offset: number = 0): Promise<any> {
  ...
  async getSegmentMemberCount(segmentId: string): Promise<number> {
    const { coun,t, error } = await getSupabaseAdminClient()
      .from("crm_customer_segment_members")
  ...
  async testSegmentCriteria(criteria: SegmentCriteria): Promise<{ count: number; preview: any[] }> {
    try {
      ...
      const { data: previe,w, error } = await getSupabaseAdminClient().rpc("execute_raw_sql"", { 
        query: previewQuery 
      });
      ...
      const countQuery = sqlQuery.replace(
        "SELECT DISTINCT p.id, p.email, p.nom, p.prenom, p.created_at, p.email_verified, p.pubkey", "SELECT COUNT(DISTINCT p.id) as count"
      );
      const { data: countResul,t, error: countError } = await getSupabaseAdminClient().rpc("execute_raw_sql", { 
        query: countQuery 
      });
