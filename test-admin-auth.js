...
async function testAdminAuth() {
  ...
  try {
    ...
    if (sendCodeResponse.ok && sendData.success) {
      ...
      console.log(`curl -X POST ${baseUrl}/api/otp/verify-code -H "Content-Type: application/json" -d '{"email":"${adminEmail}","code":"VOTRE_CODE","name":"Administrateur DazNode"}'`);
    ...
    console.log(`🔒 Dashboard admin protégé: ${dashboardResponse.status} (${dashboardResponse.status === 302 || dashboardResponse.status === 200 ? 'Protection OK' : 'Problème de protection'})`);
